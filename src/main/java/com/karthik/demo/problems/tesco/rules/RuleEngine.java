...
public class RuleEngine {
  ...
  public List<Rule> fetchRules() {
    ...
    return List.of(
        new Rule("BulkBuyLimit", "*", 10), new Rule("BulkBuyLimitCategory", "paracetamol", 5));
  }
  ...
  public String checkRules(List<Item> basketItems) {
    ...
    long count =
        basketItems.stream()
            .filter(item -> item.getQuantity() > lineItemCheckRule.getCategoryQty())
            .count();

    if (count > 0) return "BREACHED";
