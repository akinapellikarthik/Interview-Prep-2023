 ...
 *
 * Capacity: 320 * 800 = 2,56,000 requests we can expect
 *
 ...
 *
 * ClockIn and ClockOut - 8 AM to 12 PM - 2PM to 7PM
 *
 ...
 *
 *
 * for that particular empId - grab all details from the DB -   get First ClockIn time and grab the last clockOut Time
 *
 * [i,o,i,o,i,o,i,o]
 * [8,9,9.15,12,12.05,14.05,15.00,15.05,19.00]
 * [I,O,I   , O,I    , 0,I
 *
 *
 * a[i+2] - a[i+1 ]
 ...
 * shiftStart = 8 a[0]
 *
 * intermediateShiftEnd = a[i+1], intermediateShiftStart = a[i+2]
 *
 * shiftEnd = 19 a[size -1]
 * for(int i=1,j=2,i<a.size();i++,j++){
 *  //i,i+1
 *  if(a[i+1]-a[i]>= configuredValue){
 *    intermediateShiftEnd = a[i+1], intermediateShiftStart = a[i+2]
 *  }
 * }
 *
 * in middle clockOut and clockIn - if the difference between them is more than 1Hr
 *
 * clockOut - shift end
 * immediate clockIn - shift start
 *
 *
 */
...
public class TescoMain {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(String.format("His shift timings in this case are %s to %s and %s to %s.",shiftStart,intermediateShiftEnd,intermediateShiftStart,shiftEnd));
